...
contract('Voting App', ([root, holder1, holder2, holder20, holder29, holder51, nonHolder]) => {
    ...
    for (const decimals of [0, 2, 18, 26]) {
        context(`normal token supply, ${decimals} decimals`, () => {
            ...
            it('deciding voting is not automatically executed', async () => {
                ...
                assert.equal(await executionTarget.counter(), 0, 'should have not received execution call')
            })

            it('voting is executed after voting duration', async () => {
                const action = { to: executionTarget.address, calldata: executionTarget.contract.execute.getData() }
                const script = encodeCallScript([action])
                const voteId = createdVoteId(await voting.newVote(script, '', true, { from: holder51 }))

                await voting.mockSetTimestamp(NOW + votingDuration)
                await voting.executeVote(voteId)

                assert.equal(await executionTarget.counter(), 1, 'should have received execution call')
            })

            it('deciding voting is not automatically executed (long version)', async () => {
                const action = { to: executionTarget.address, calldata: executionTarget.contract.execute.getData() }
                const script = encodeCallScript([action])
                await voting.newVoteExt(script, '', true, { from: holder51 })
                assert.equal(await executionTarget.counter(), 0, 'should have not received execution call')
            })
            ...
            context('creating vote', () => {
                ...
                beforeEach(async () => {
                    ...
                    const receipt = await voting.newVoteExt(script, 'metadata', false, { from: holder51 });
                ...
                it('cannot vote on executed vote', async () => {
                    ...
                    await voting.mockIncreaseTime(votingDuration + 1)
                    await voting.executeVote(voteId)
                    await assertRevert(voting.vote(voteId, true, { from: holder20 }), errors.VOTING_CAN_NOT_VOTE)
                })
            })
        })
    }
    ...
    context('token supply = 1', () => {
        ...
        it('new vote cannot be executed before voting', async () => {
            ...
            assert.isTrue(isOpen, 'vote should not be closed')
        ...
        context('new vote parameters', () => {
            it('creating vote as holder executes vote (if _canExecute param says so)', async () => {
                const voteId = createdVoteId(await voting.newVoteExt(EMPTY_SCRIPT, 'metadata', true, { from: holder1 }))
                const [isOpen, isExecuted] = await voting.getVote(voteId)
    ...
    context('token supply = 3', () => {
        ...
        it('new vote cannot be executed before voting duration and can be executed after voting duration', async () => {
            const voteId = createdVoteId(await voting.newVote(EMPTY_SCRIPT, 'metadata', true))
            ...
            let [isOpen, isExecuted] = await voting.getVote(voteId)
            ...
            let [isOpenAfterExecution, isExecutedAfterExecution] = await voting.getVote(voteId)

            assert.isFalse(isOpenAfterExecution, 'vote should be closed')
            assert.isTrue(isExecutedAfterExecution, 'vote should have not been executed')
        })
        ...
        it('creating vote as holder2 does not execute vote', async () => {
            ...
            assert.isTrue(isOpen, 'vote should be closed')
            assert.isFalse(isExecuted, 'vote should have been executed')
        })
    })
